...
(function(){
    ...
    function insertSingle(){
        var width = $G("videoWidth"),
            ...
            align = findFocus("videoFloat","name");
        if(!url) return false;
        if(url.indexOf('qq.com') == -1){return false;}// 只处理腾讯视频
        if ( !checkNum( [width, height] ) ) return false;
    ...
    function convert_url(url){
        if ( !url ) return '';
        //var currentUser = window.top.document.getElementById('currentUser').value || '';
        var vid = null;
        $.ajax({
        	url: '/intemarket/ueditor/parseVideoVid.do',
        	type: 'post',
        	dataType: 'json',
        	data:{url: url},
        	async: false,
        ...
        if(!vid || vid.indexOf('http://') != -1) {
        	...
        	vid = 'https://imgcache.qq.com/tencentvideo_v1/playerv3/TPout.swf?vid='+vid;
        }
        return vid;
    }
    ...
    function createPreviewVideo(url){
        ...
        $G("preview").innerHTML = '<div class="previewMsg"><span>视频解析中...</span></div>';
        $('#video').data('vid', null);
        var conUrl = convert_url(url);
        $('#video').data('vid', conUrl);
        $G("preview").innerHTML = '<div class="previewMsg" id="_previewMsg"><span>视频加载中...</span></div>'+
        '<embed id="_preview_video" onerror="onVideoLoadError()" class="previewVideo" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
	        ' src="' + conUrl + '"' +
	        ' width="' + 420  + '"' +
	        ' height="' + 280  + '"' +
	    ...
	    '</embed>';   
        setTimeout(function(){
        	$('#_previewMsg').html('<span>'+lang.urlError+'</span>');
        }, 300000);
    }
